...
class UserService:
    ...
    async def get_users(self, page: int = 1, size: int = 10):
        try:
            ...
            return {"users": users, "page": page, "size": size, "pages_count": pages_count, "users_count": users_count}

        except Exception as e:
            await async_log(f"Failed to get users list: {e}")
            raise HTTPException(status_code=404, detail=f"Failed to get users list: {e}")
    ...
    async def create_user(self, user: UserSignUpSchema):
        try:
            ...
            if existing_user:
                ...
                raise HTTPException(status_code=400, detail=f"User with email {user.email} is already registered.")

            db_user = await self.user_repository.save_user(created_user)

            await async_log(f"Creating user {db_user.email} (ID {db_user.id}) was successful.")
            return db_user
        except Exception as e:
            await async_log(f"Failed to create user {user.email}: {e}")
            raise HTTPException(status_code=400, detail=f"Failed to create user {user.email}: {e}")
    ...
    async def update_user(self, id: int, update_data: UserUpdateSchema):
        try:
            ...
            if not db_user:
                ...
                raise HTTPException(status_code=404, detail="User with that id is not registered.")
