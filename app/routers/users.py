...
class UserRouter:
    def __init__(self):
        ...
        self.router.add_api_route(
            "/", self.get_users, methods=["GET"], response_model=UserListSchema
        )
        self.router.add_api_route(
            "/{id}/", self.get_user, methods=["GET"], response_model=UserDetailSchema
        )
        self.router.add_api_route(
            "/", self.create_user, methods=["POST"], response_model=UserDetailSchema
        )
        self.router.add_api_route(
            "/{id}/update/",
            self.update_user,
    ...
    async def get_users(
        ...
        users = [
            UserBaseSchema.model_validate(user.__dict__)
        ...
        return UserListSchema(
            ...
            next_page=(
                ...
                if page < response.get("pages_count")
                else None
            ),
    ...
    async def get_user(
        self, id: int, user_service: UserService = Depends(get_user_service)
    ):
